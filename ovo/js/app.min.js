$(function () {
    ...
    function getRandom(lo, hi) {
        return lo = lo || 100, hi = hi || 0,  // 默认范围为 0-100
            Math.ceil(Math.random() * (lo - hi) + hi)
    }
    ...
    function n(t, e) {
        if (!state) {
            if (d = t, time.text(m[d][0]), s.text("神马"), c.text("？"), title.addClass("shake"), cnt = 0, e) {
                var n = $(".tip").show();
                setTimeout(function () {
                    n.remove()
                }, 4e3)
    ...
    var state = 0;               // 控制切换状态
    var title = $(".title");
    var time = $(".time");       // 显示时间区域(早饭/午饭/晚饭)
    var s = $(".what");          // 显示食物名称
    var c = $(".punctuation");
    var l = $("#start");
    var r = $("#temp_container");
    var m = [["早饭", 
        "面包 蛋糕 荷包蛋 烧饼 饽饽 肉夹馍 油条 馄饨 火腿 面条 小笼包 玉米粥 肉包 煎饼果子 饺子 煎蛋 烧卖 生煎 锅贴 包子 酸奶 苹果 梨 香蕉 皮蛋瘦肉粥 蛋挞 南瓜粥 煎饼 玉米糊 泡面 粥 馒头 燕麦片 水煮蛋 米粉 豆浆 牛奶 花卷 豆腐脑 煎饼果子 小米粥 黑米糕 鸡蛋饼 牛奶布丁 水果沙拉 鸡蛋羹 南瓜馅饼 鸡蛋灌饼 奶香小馒头 汉堡包 披萨 八宝粥 三明治 蛋包饭 豆沙红薯饼 驴肉火烧 粥 粢饭糕 蒸饺 白粥".split(" ")],
        ["午饭"],
    ...
    var h = "冰箱 书桌 电扇 空调 马桶 翔 鼠标 键盘 显示器 电视 台灯 饭盒 iPad iPhone 手机 餐巾纸 电话 椅子 纸箱 窗帘 插座 被单 报纸 杂志 相框 照片 衣服 内裤 内衣 袜子 妹子 汉子 砖头 混凝土 钢筋 塑料袋 衣架 书 手环 手表 鼠标垫 眼药水 跑车 自行车 三轮车 坦克 潜水艇 飞机 火箭 U盘 CPU 显卡 刀片 碎玻璃 圆珠笔 钢笔 交通卡 银行卡 身份证 户口簿 橡皮筋 双面胶 502胶水 订书机 螺丝刀 锤子 榔头 垃圾桶 花花草草 树皮 洗手液 妇炎洁 姨妈巾 哆啦A梦 仙人掌 企鹅 大熊猫 穿山甲 米老鼠 唐老鸭 跳跳虎 旅行箱 DVD 音响 热水器 热水袋 电热棒 电池 充电器 相机 自拍杆 耳机 吊灯 雨伞 钱包 鞋子 人字拖 床垫 绣花针 戒指 窨井盖 路灯 主板 程序猿 工程狮 电线 摄像头 西北风 生活 路由器 洗手液 沐浴露 肥皂 羽毛球拍 保龄球 皮带 皮鞭 电池 牙膏 手电筒 瑜伽垫 假发 82年的自来水 马蜂窝 瑞士军刀 地板 水管 电钻".split(" "),
    ...
    title.on("animationend webkitAnimationEnd", function (t) {
        ...
        if (g.toggleClass("playing", !state), state)
            state = 0,
                "monster" == w && x.text(u[getRandom(u.length)]),
                c.text("！"),
                l.find("span").text("换一个"),
                clearTimeout(b),
                document.title = "小明の雜貨鋪｜" + title.text() + "這是一個很無聊的網站，但能解決你的人生一大困擾！選擇困難戶必備！";
        else {
            state = 1,
                cnt++,
                2 == cnt && e("我就知道你會換一個 ( ͡° ͜ʖ ͡°)"),
                5 == cnt && e("說，你是不是天秤座？Σ( ° △ °|||)︴"),
                10 == cnt && e("你是吃了炫邁嗎？(￣△￣；)"),
                20 == cnt && e("難道你是處女座？ (๑•̀ㅂ•́)و✧"),
                30 == cnt && e("再換我可要報警了！( *・ω・)✄╰ひ╯");
            var n = "human" == w ? m[d][1] : h;
            c.text("？"), l.find("span").text("停"), x.text(""), function time() {
                var e = getRandom(n.length) - 1, state = n[e], title = getRandom(window.innerHeight), c = getRandom(window.innerWidth - 50), l = getRandom(37, 14);
                s.text(state).css({
                    'white-space': 'normal',  
                    'overflow': 'visible',
                    'text-overflow': 'clip',
                ...
                var m = $("<span class='temp'>" + state + "</span>").css({
                    top: title, left: c, color: "rgba(0,0,0," + getRandom(7, 3) / 10 + ")", fontSize: l + "px"
                }).appendTo(r);
                b = setTimeout(time, 60)
            }()
        }
    }),
    $(document).ready(function() {
        $(".title").off('click').on('click', function() {
            ...
            // showFloatingNotice("切换为" + m[(d >= m.length - 1 ? 0 : d + 1)][0] + "!", 1500);
        });
    });
    ...
    9 > p || p >= 23 ? n(0, 1) : 13 > p && n(1, 1), title.click(function () {
        n(d >= m.length - 1 ? 0 : d + 1)
    }),
